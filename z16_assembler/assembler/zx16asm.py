        ...
        if mnemonic == 'li16':
            ...
            if isinstance(label, str):
                if symbol_resolver:
                    ...
                    # Calculate offset from NEXT instruction (auipc uses current PC)
                    offset = label_addr - (current_pc ) & 0xFFFF

                    # Split into upper and lower parts
                    high = (offset + 0x40) >> 7  # 0x400 for proper rounding
                    low = offset - (high << 7)

                    # Ensure low fits in 7-bit signed immediate (-64 to 63)
                    if low < -64:
                        high += 1
                        low = offset - (high << 7)
                    elif low > 63:
                    ...
                    # Verify low is in range
                    if low < -64 or low > 63:
                        raise ValueError(f"Cannot represent offset {offset} with 7-bit low part")

                    # Mask to required bit widths
                    high &= 0x1FF  # 9 bits
                    low &= 0x7F    # 7 bits (sign-extended later during encoding)
                ...
                offset = label - (current_pc ) & 0xFFFF
                high = (offset + 0x40) >> 7
                low = offset - (high << 7)
