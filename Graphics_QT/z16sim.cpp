...
z16sim::z16sim(QWidget *parent)
    : QMainWindow(parent), pc(0), debug(false), verbose(false)
{
    std::memset(regs, 0, sizeof(regs));
    std::memset(memory, 0, sizeof(memory));
    std::fill(std::begin(infinityCheck), std::end(infinityCheck), 0);
}
z16sim::z16sim(bool debugMode) : pc(0), debug(debugMode), verbose(false) {
    std::memset(regs, 0, sizeof(regs));
    std::memset(memory, 0, sizeof(memory));
}
...
constexpr int TILES_X = SCREEN_WIDTH / TILE_SIZE;  // 20
constexpr int TILES_Y = SCREEN_HEIGHT / TILE_SIZE; // 15

constexpr int TILEMAP_ADDR = 0xF000;
constexpr int TILEMAP_SIZE = TILES_X * TILES_Y; // 300 bytes

constexpr int TILESET_ADDR = 0xF200;
constexpr int TILE_DEFINITION_SIZE = 128;  // bytes per tile
constexpr int TILE_COUNT = 16;

constexpr int PALETTE_ADDR = 0xFA00;

constexpr int PALETTE_SIZE = 16;  // 16 colors
QColor decodeColor(quint8 byte) {
    int r = ((byte >> 5) & 0x07) * 255 / 7;  // 3 bits
    int g = ((byte >> 2) & 0x07) * 255 / 7;  // 3 bits
    int b = (byte & 0x03) * 255 / 3;         // 2 bits
    return QColor(r, g, b);
}
QImage z16sim::renderTiledScreen() {
    constexpr int SCREEN_WIDTH = 320;
    constexpr int SCREEN_HEIGHT = 240;
    constexpr int TILE_SIZE = 16;
    constexpr int TILES_X = SCREEN_WIDTH / TILE_SIZE;
    constexpr int TILES_Y = SCREEN_HEIGHT / TILE_SIZE;
    constexpr int TILEMAP_ADDR = 0xF000;
    constexpr int TILESET_ADDR = 0xF200;
    constexpr int TILE_DEFINITION_SIZE = 128;
    constexpr int PALETTE_ADDR = 0xFA00;
    ...
    for (int ty = 0; ty < TILES_Y; ++ty) {
        for (int tx = 0; tx < TILES_X; ++tx) {
            int tileIndex = memory[TILEMAP_ADDR + ty * TILES_X + tx];

            for (int py = 0; py < TILE_SIZE; ++py) {
                for (int px = 0; px < TILE_SIZE; ++px) {
                    int pixelOffset = py * TILE_SIZE + px;
                    int colorIndex = memory[TILESET_ADDR + tileIndex * TILE_DEFINITION_SIZE + pixelOffset];
                    int colorByte = memory[PALETTE_ADDR + (colorIndex & 0x0F)];
                    QColor color = decodeColor(colorByte);
                    img.setPixelColor(tx * TILE_SIZE + px, ty * TILE_SIZE + py, color);
                }
            }
        }
    }
...
z16sim::~z16sim() {
    // Clean up if needed. If nothing is dynamically allocated, leave empty.
}
